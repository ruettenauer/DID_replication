...
test0 <- sim_did(
  ...
  beta.t = rep(0.1, 15), # Temporal distribution of treatment effect
...
save(test0, file = "Hettreat_Sim0.RData")





### Test 0b - normal TWFE with homogeneous treatment effect, group-specific
...
save(test0b, file = "Hettreat_Sim0b.RData")
...
save(test1, file = "Hettreat_Sim1.RData")
...
save(test8, file = "Hettreat_Sim8.RData")
...
test9 <- sim_did(
  ...
  b.nonparallel = c(0.00), # Extra trend for treated (vs. non-treated) # ADD Extra trend for early treated vs late?
  beta = 1, # Treatment effect (and its distribution)
  beta.t = cumsum(c(0.05, rep(0.03214285, 15-1))), # Temporal distribution of treatment effect
  beta.late = 0.5, # early vs late with heterogeneous ATT, factor for late (early will have 1/beta.late), 
  size.late = 0.5, # share of late treatment group in case of beta.late 
  crs = ncores, #Parallel cores
)

save(test9, file = "Hettreat_Sim9.RData")
...
### Test 9 - normal TWFE with trend-breaking treatment effect -- no group-specific treatment effect
...
save(test9a, file = "Hettreat_Sim9a.RData")
...
save(test10, file = "Hettreat_Sim10.RData")
...
save(test12, file = "Hettreat_Sim12.RData")
...
save(test13, file = "Hettreat_Sim13.RData")
...
save(test14, file = "Hettreat_Sim14.RData")
