...
fm <- as.formula(lifesat ~ has_child 
                 + hhincome + married + divorced + cohab + health_issue + 
                   owner + unemployed)

### Only below 50
ukhls.df <- ukhls.df[which(ukhls.df$age <= 50), ]
...
### Model TWFE
out.fect1 <- feols(lifesat ~ time_to_treatment 
                   + hhincome + married + divorced + cohab + health_issue + 
...
out.fect1.simple <- feols(lifesat ~ has_child 
                   + hhincome + married + divorced + cohab + health_issue + 
                     owner + unemployed | pidp + wave, 
                   data = ukhls.df)
...
out.did <- att_gt(
  ...
  idname = "pidp",
  tname = "wave",
  xformla = ~ hhincome + married + divorced + cohab + health_issue + owner + unemployed,
...
out.sa <-
  feols(lifesat ~ sunab(Treat_first, wave, ref.p = -c(1 + anticipation)) + 
          hhincome + married + divorced + cohab + health_issue + 
          owner + unemployed | pidp + wave, 
        data = ukhls.df)
summary(out.sa)
...
out.bo <- did_imputation(
  ...
  tname =  "wave",
  idname = "pidp",
  first_stage = ~ hhincome + married + divorced + cohab + health_issue + 
    owner + unemployed | pidp + wave,
  horizon = TRUE,
  pretrends = c(-10:-3, -1) ## setting true --> add zero period below
)
out.bo
...
#--- Model 7 Wooldridge
out.etwfe <- etwfe(
  fml  = lifesat ~ hhincome + married + divorced + cohab + health_issue + 
    owner + unemployed,
  tvar = "wave",
  gvar = "Treat_first_ant",
  data = ukhls.df
)
...
save(out.fect3, file = "MC_est.RData")
...
res.etwfe <- data.frame(time = as.numeric(es.etwfe$event) - 1,
...
### Save
save(res.df, file = "Case_study_event_1.RData")
save(sum.df, file = "Case_study_simple_1.RData")
...
# Restrict window
t <- 6
mod_es <- mod_es[which(mod_es$time <= 6 & mod_es$time >= -4),]
...
png(filename = "Case_Lifesat_att.png",
...
png(filename = "Case_Lifesat_avg.png",
...
png(filename = "Case_Lifesat_comb.png",
